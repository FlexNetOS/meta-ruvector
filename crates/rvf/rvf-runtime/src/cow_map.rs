...
impl CowMap {
    ...
    // Canonical CoW map codec, restored from upstream. This fork had relocated
    // cluster-map encoding into rvf_wire::{encode_cow_map, decode_cow_map},
    // which frames a versioned CowMapHeader for the on-disk segment payload.
    // That framing is additive and stays, but it is a different layer: the CoW
    // map type must still own its own flat wire format rather than have it
    // reinvented elsewhere, so serialize/deserialize live here as upstream
    // defines them.
