...
mod tests {
    ...
    fn read_returns_synthetic_zones_in_order() {
        ...
        let snap = sensor.read().expect("synthetic thermal read");
    ...
    fn apply_profile_writes_target_to_each_policy() {
        ...
        let snap = sensor.read().expect("synthetic thermal read after profile");
        assert_eq!(snap.cpu_policies.len(), 2);
    ...
    fn read_handles_missing_roots_gracefully() {
        ...
        let snap = sensor.read().expect("missing thermal roots read");
