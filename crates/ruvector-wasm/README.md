...
**Ruvector WASM brings vector search to the browser:**

- 🔒 **Privacy First**: All data stays in the browser—zero server round-trips
- 🌐 **Edge Computing**: Deploy to CDNs and edge runtimes
- 📦 **Compact**: <400KB gzipped with optimizations
- 🎯 **Pure Rust → WASM**: `ruvector-core` compiled to WebAssembly

> ⚠️ **The current WASM build uses a flat (brute-force) index, not HNSW.** It is
> compiled **without** the `hnsw` cargo feature, so search is **O(n)**, not
> O(log n). Query latency scales linearly with the number of vectors — the
> "<1ms with HNSW" figure does **not** apply to this build. See [Roadmap](#-roadmap--wip).

## 🚀 Features

### Core Capabilities

- **VectorDB API**: `insert`, `insertBatch`, `search`, `get`, `delete`, `len`, `isEmpty`, `dimensions`
- **Flat (brute-force) Index**: exact nearest-neighbor over all vectors (O(n) per query in this build)
- **Multiple Distance Metrics**: Euclidean, Cosine, Dot Product, Manhattan (set in the constructor)
- **SIMD Acceleration**: WASM SIMD distance where the build/runtime supports `simd128` (`detectSIMD()`)
- **Type-Safe**: TypeScript definitions generated by `wasm-bindgen`

### Companion JavaScript Helpers (shipped in this package)

This crate ships plain-JS helpers next to the WASM module — they wrap the WASM
`VectorDB` rather than living inside the Rust binary:

- **`adapter.js`** (`RuvectorWasmAdapter`): metadata sidecar + `similarity = 1 - distance` (recommended; see below)
- **`worker-pool.js`** (`WorkerPool`): runs `VectorDB` calls inside Web Workers you create
- **`indexeddb.js`** (`IndexedDBPersistence`): JS-side IndexedDB read/write of vector entries

> The Rust-side `saveToIndexedDB()` / `loadFromIndexedDB()` exports are **stubs**:
> `saveToIndexedDB()` only logs and resolves, and `loadFromIndexedDB()` rejects
> with "Not yet implemented". Real persistence is done by the `indexeddb.js`
> helper, not by those methods.

### Performance Notes

- **Batch Insert**: `insertBatch` avoids per-call boundary crossings for bulk loads
- **WASM Streaming**: Compile WASM modules while downloading (host/loader concern)

> The LRU cache, progressive loading, and compressed IndexedDB serialization
> mentioned in some examples live in the **`indexeddb.js` helper**, not in the
> Rust/WASM core.

## 🛣️ Roadmap / WIP

These are **not** in the current build — they are planned:

- **HNSW in WASM** — the headline "<1ms / O(log n)" search depends on compiling
  `ruvector-core` with the `hnsw` feature for `wasm32`. Today the build uses
  `memory-only` + flat index, so search is O(n).
- **Native IndexedDB persistence** — the Rust `saveToIndexedDB()` /
  `loadFromIndexedDB()` methods are stubs. Use the `indexeddb.js` helper today.
- **Metadata round-trip via raw bindings** — use the `adapter.js` sidecar.
...
- Query latency: scales with corpus size (flat O(n) scan in this build)
- Memory footprint: ~60MB

> These numbers are indicative for the **flat-index** build. They will improve
> once the WASM HNSW lands (see Roadmap). Always benchmark with your own data.
