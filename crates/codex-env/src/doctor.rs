...
fn validate_agent_teams(
    ...
    for team in &manifest.teams {
        ...
        if !team.members.is_empty() && team.members != team.agents {
            bail!(
                "{} agent team {} has members that diverge from agents",
                path.display(),
                team.name
            );
        }
...
fn validate_automation_graph(
    codex_dir: &Path,
    agent_teams: usize,
    agent_team_members: usize,
) -> Result<()> {
    let path = codex_dir.join("automation-graph.json");
    let graph: serde_json::Value = serde_json::from_slice(
        &fs::read(&path).with_context(|| format!("failed to read {}", path.display()))?,
    )
    .with_context(|| format!("failed to parse {}", path.display()))?;
    if graph.get("generatedBy").and_then(serde_json::Value::as_str) != Some("codex-env") {
        bail!("{} must be generated by codex-env", path.display());
    }
    if graph.get("sourceRoot").and_then(serde_json::Value::as_str) != Some(".claude") {
        bail!("{} must record .claude as sourceRoot", path.display());
    }
    if graph
        .pointer("/ownership/rustExtractor")
        .and_then(serde_json::Value::as_str)
        != Some("crates/codex-env")
    {
        bail!(
            "{} must record crates/codex-env as the Rust extractor",
            path.display()
        );
    }
    if graph
        .pointer("/ownership/notTarget")
        .and_then(serde_json::Value::as_str)
        != Some("vendor harness")
    {
        bail!("{} must reject vendor harness ownership", path.display());
    }
    let graph_teams = graph
        .pointer("/agents/teams")
        .and_then(serde_json::Value::as_array)
        .ok_or_else(|| anyhow!("{} must contain agents.teams", path.display()))?;
    if graph_teams.len() != agent_teams {
        bail!(
            "{} team count {} does not match agent-teams.json count {}",
            path.display(),
            graph_teams.len(),
            agent_teams
        );
    }
    let graph_members = graph_teams
        .iter()
        .filter_map(|team| team.get("members").and_then(serde_json::Value::as_array))
        .map(Vec::len)
        .sum::<usize>();
    if graph_members != agent_team_members {
        bail!(
            "{} team member count {} does not match agent-teams.json count {}",
            path.display(),
            graph_members,
            agent_team_members
        );
    ...
    if graph
        .pointer("/tokenLoadControls/rawMirrorPolicy")
        .and_then(serde_json::Value::as_str)
        .is_none_or(|policy| !policy.contains("evidence-only"))
    {
        bail!(
            "{} must mark the raw mirror as evidence-only for token load control",
            path.display()
        );
    }
    Ok(())
...
fn validate_prompts(
    ...
    for source in &source_prompts {
        if !source.starts_with(&source_dir) {
            bail!(
                "Codex prompt {} is outside repo-local prompt surface {}",
                strip_repo_prefix(repo_root, source).display(),
                strip_repo_prefix(repo_root, &source_dir).display()
            );
        }
    }
    Ok((
