#!/usr/bin/env node
// Slice 10b scanner: prime-radiant + ruvector-nervous-system + ruvector-nervous-system-wasm + sona
// Uses slice-10b-files.txt as authoritative input list.
...
if (!projectRoot || !outPath) {
  console.error('Usage: node ua-slice10b-scan.js <project-root> <out-json>');
  process.exit(1);
}

const filesListPath = path.join(projectRoot, '.understand-anything/tmp/slice-10b-files.txt');
if (!fs.existsSync(filesListPath)) {
  console.error('Missing slice-10b-files.txt at: ' + filesListPath);
  process.exit(1);
}
...
// Use prime-radiant as project name and description source (largest / first crate)
const primeReadme = path.join(projectRoot, 'crates/prime-radiant/README.md');
...
const primeCargo = path.join(projectRoot, 'crates/prime-radiant/Cargo.toml');
...
const result = {
  ...
  slice: '10b',
  scope: 'crates/prime-radiant + crates/ruvector-nervous-system + crates/ruvector-nervous-system-wasm + crates/sona',
  name: projectName,
  rawDescription,
  readmeHead,
  languages,
