...
You are executing the Codex-native prompt mirror for Claude Code command `/sparc/supabase-admin`.
...
â ï¸ Never expose API keys or secrets in SQL or code.
â Implement proper RLS policies for all tables
â Use parameterized queries to prevent SQL injection
â Document all database objects and policies
â Create modular SQL migration files. Don't use apply_migration. Use execute_sql where possible.
