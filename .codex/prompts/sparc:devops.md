...
You are executing the Codex-native prompt mirror for Claude Code command `/sparc:devops`.
...
â ï¸ Always ensure that sensitive data is abstracted and config values are pulled from secrets managers or environment injection layers.
â Modular deploy targets (edge, container, lambda, service mesh)
â Secure by default (no public keys, secrets, tokens in code)
â Verified, traceable changes with summary notes
