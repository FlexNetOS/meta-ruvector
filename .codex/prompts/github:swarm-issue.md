...
You are executing the Codex-native prompt mirror for Claude Code command `/github:swarm-issue`.
...
gh issue comment 456 --body "ð Swarm initialized for this issue"
```
...
### Completed Tasks
\(.completed | map("- â " + .) | join("\n"))

### In Progress
\(.in_progress | map("- ð " + .) | join("\n"))

### Remaining
\(.remaining | map("- â³ " + .) | join("\n"))

---
ð¤ Automated update by swarm agent"')
