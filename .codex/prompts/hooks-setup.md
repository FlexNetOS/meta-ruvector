...
You are executing the Codex-native prompt mirror for Claude Code command `/hooks/setup`.
...
This command documents how to (re)wire that stack **and** the four integrated tools â
...
tools' real hook surfaces â verify with `<tool> hook --help` / `<tool> --help`.

> â ï¸ `npx claude-flow init --hooks` **overwrites** `.claude/settings.json`. This repo's
> config is hand-tuned â back it up first (`cp .claude/settings.json{,.bak}`) or merge
...
npx claude-flow init --hooks            # â ï¸ rewrites settings.json â back up first

# ICM â Infinite Context Memory (persistent cross-session memory)
icm init --mode hook                    # hooks only; use --mode standard for cli+skill+hook

# RTK â Rust Token Killer (auto-rewrites verbose commands to compact equivalents)
rtk init                                # installs the PreToolUse rewrite hook

# grit â symbol locks + worktree coordination for multi-agent work
grit init                               # initialize grit in the repo (then wire hooks below)

# vox â spoken feedback (TTS)
...
## RTK hooks â command rewriting

RTK rewrites verbose commands to compact equivalents (60â90% token savings) at the
...
|-------|---------|---------|---------|
| `PreToolUse` | `Bash` | `rtk hook claude` | rewrite `git status` â `rtk git status`, etc. (0-token overhead, transparent) |
...
## grit hooks â symbol locks & multi-agent coordination

grit has **no native `grit hook` subcommand** â wire its primitives into events. Use this
when multiple agents edit the same repo concurrently (claim symbols â heartbeat â release).
...
> (merge worktree + release all locks) â keep that an agent action, not a hook.

---

## vox hooks â spoken feedback
...
- Keep spoken hooks short â TTS for trivial ops is noise. Prefer agents calling
  `vox say` deliberately for real summaries over a chatty `Stop` hook.

---

## Combined example â all tools coexisting
...
- `continue` â proceed (true/false)
- `reason` â explanation for the decision
...
- Don't speak (vox) on trivial ops â reserve it for real summaries.
