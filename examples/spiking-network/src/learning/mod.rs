...
impl STDPLearning {
    ...
    pub fn compute_delta(&self, delta_t: f32) -> f32 {
        ...
        let tau = self.config.tau_pre.min(self.config.tau_post);
        if delta_t > 0.0 {
            w_max * delta_t / tau
        } else {
            -w_max * (-delta_t) / tau
    ...
    pub fn apply(&self, weight: f32, delta_t: f32) -> f32 {
        (weight + self.config.lr * self.compute_delta(delta_t))
            .max(0.0)
            .min(self.config.w_max)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_config() {
        assert!(STDPConfig::default().lr > 0.0);
    }
    #[test]
    fn test_positive_delta() {
        let l = STDPLearning::new(STDPConfig::default());
        assert!(l.compute_delta(5.0) > 0.0);
    }
    #[test]
