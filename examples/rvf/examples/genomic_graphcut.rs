...
fn generate_chromosome(n: usize, plat: Platform, seed: u64) -> Vec<Window> {
    ...
    let drvs = drivers();
    let gc_wave =
        |i: usize| 0.42 + 0.08 * (i as f64 * 0.01).sin() + 0.04 * (i as f64 * 0.037).cos();
    ...
    for i in 0..n {
        ...
        if anom == Anomaly::Normal {
            ...
            if r < 0.02 {
                anom = Anomaly::Gain;
                cn = 3.0 + lcg_f64(&mut rng);
            } else if r < 0.04 {
                anom = Anomaly::Loss;
                cn = 0.5 + lcg_f64(&mut rng) * 0.5;
            } else if r < 0.05 {
                anom = Anomaly::Loh;
            } else if r < 0.06 {
                anom = Anomaly::Hotspot;
            } else if r < 0.065 {
                anom = Anomaly::Sv;
            }
        }
...
fn extract_embedding(w: &Window, plat: Platform) -> Vec<f32> {
    ...
    vec![
        log2r as f32,
        (w.read_depth / exp) as f32,
        w.read_depth.sqrt() as f32 / 10.0,
        (w.read_depth - exp).abs() as f32 / exp as f32,
        w.baf as f32,
        baf_d as f32,
        (baf_d * 4.0).min(1.0) as f32,
        if baf_d > 0.15 { 1.0 } else { 0.0 },
        gc_nd as f32,
        w.gc as f32,
        (gc_nd - 1.0).abs() as f32,
        gc_nd.log2().abs() as f32,
        vr as f32,
        (vr / 3.0).min(3.0) as f32,
        if w.var_count > 15 { 1.0 } else { 0.0 },
        w.mq as f32 / 60.0,
        (log2r * baf_d) as f32,
        (log2r.abs() + vr / 5.0) as f32,
        (if log2r > 0.3 {
            1.0
        } else if log2r < -0.3 {
            -1.0
        ...
        pc,
        (w.read_depth / 500.0).min(1.0) as f32,
        w.index as f32 / 1000.0,
        (w.index as f64 * 0.01).sin() as f32,
        (log2r * log2r) as f32,
        (baf_d * baf_d) as f32,
        (log2r * gc_nd) as f32,
        (vr * baf_d) as f32,
        (log2r.abs() * w.mq / 60.0) as f32,
        ((gc_nd - 1.0) * log2r) as f32,
        (w.var_count as f32).sqrt() / 5.0,
        0.0,
    ]
}
...
fn unary_score(w: &Window, plat: Platform) -> f64 {
    ...
    let gc_penalty = if w.gc < 0.30 || w.gc > 0.65 {
        0.15
    } else {
        0.0
    };
    ...
    for (i, w) in wins.iter().enumerate() {
        ...
        if calls[i] && truth {
            ...
            let tc = match w.truth {
                Anomaly::Gain => Call::Gain,
                Anomaly::Loss => Call::Loss,
                Anomaly::Loh => Call::Loh,
                _ => Call::Normal,
            };
            if types[i] == tc {
                cc += 1;
    ...
    for d in &drvs {
        ...
        for i in 1..calls.len() {
            if calls[i] != calls[i - 1] {
                best = best
                    .min((i as isize - d.start as isize).unsigned_abs())
                    .min((i as isize - d.end as isize).unsigned_abs());
            }
        }
        bpe.push(best as f64);
    }
    let bp = if bpe.is_empty() {
        0.0
    } else {
        1.0 / (1.0 + bpe.iter().sum::<f64>() / bpe.len() as f64)
    };
...
fn main() {
    ...
    for (pi, &plat) in platforms.iter().enumerate() {
        ...
        println!(
            ...
            n_win,
            n_ab,
            n_ab as f64 / n_win as f64 * 100.0
        );
        for a in &[
            Anomaly::Gain,
            Anomaly::Loss,
        ...
        println!(
            "\n  Graph Cut:  sens={:.3} spec={:.3} bp={:.3} class={:.3}",
            gs, gp, gb, ga
        );
        println!(
            "  Threshold:  sens={:.3} spec={:.3} bp={:.3} class={:.3}",
            ts, tp_, tb, ta
        );
        println!("\n  Driver Gene Detection:");
        println!(
            "    {:>6} {:>8} {:>6} {:>8} {:>5}",
            "Gene", "Region", "Truth", "Called", "Det"
        );
        for d in &drvs {
            let cc = (d.start..d.end).filter(|&i| gc[i]).count();
            let sz = d.end - d.start;
            let det = if cc as f64 / sz as f64 > 0.5 {
                "YES"
            } else {
                "no"
            };
    ...
    let entries: Vec<WitnessEntry> = steps
        ...
        .map(|(i, (s, wt))| WitnessEntry {
            prev_hash: [0u8; 32],
            action_hash: shake256_256(format!("gc:{}:{}", s, i).as_bytes()),
    ...
    let child = store
        .derive(
            &tmp.path().join("cnv_report.rvdna"),
    ...
    println!("\n=== Summary ===");
    println!(
        "  {} windows x 3 platforms = {} vectors",
        n_win, ing.accepted
    );
    println!(
        "  Drivers: {} genes | Witness: {} steps",
        drvs.len(),
        ver.len()
    );
    println!(
    ...
    println!(
        "  Threshold: lambda_i > 2*gamma*alpha = {:.2}",
        2.0 * gamma * alpha
    );
