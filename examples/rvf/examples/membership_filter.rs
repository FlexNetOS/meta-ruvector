...
fn main() {
    ...
    println!(
        "  Mode:            {:?} (visible iff bit is set)",
        include_filter.mode()
    );
    println!(
        "  Members:         {} (empty = empty view, fail-safe)",
        include_filter.member_count()
    );
    println!();
    ...
    println!(
        "    Vector 0:      {} (included)",
        include_filter.contains(0)
    );
    println!(
        "    Vector 250:    {} (included)",
        include_filter.contains(250)
    );
    println!(
        "    Vector 499:    {} (included)",
        include_filter.contains(499)
    );
    println!(
        "    Vector 500:    {} (excluded)",
        include_filter.contains(500)
    );
    println!(
        "    Vector 999:    {} (excluded)",
        include_filter.contains(999)
    );
    println!();
    ...
    println!(
        "    Vector 249:    {} (still included)",
        include_filter.contains(249)
    );
    println!();
    ...
    println!(
        "  Mode:            {:?} (visible iff bit is NOT set)",
        exclude_filter.mode()
    );
    println!(
        "  Members:         {} (empty = full view)",
        exclude_filter.member_count()
    );
    println!();
    ...
    println!(
        "    Vector 0:      {} (all visible)",
        exclude_filter.contains(0)
    );
    println!(
        "    Vector 999:    {} (all visible)",
        exclude_filter.contains(999)
    );
    println!();
    ...
    println!(
        "    Members:       {} (excluded count)",
        exclude_filter.member_count()
    );
    println!(
        "    Vector 0:      {} (still visible)",
        exclude_filter.contains(0)
    );
    println!(
        "    Vector 899:    {} (still visible)",
        exclude_filter.contains(899)
    );
    println!(
        "    Vector 900:    {} (now hidden)",
        exclude_filter.contains(900)
    );
    println!(
        "    Vector 999:    {} (now hidden)",
        exclude_filter.contains(999)
    );
    println!();
    ...
    // Deserialize
    let restored =
        MembershipFilter::deserialize(&bitmap_data, &header).expect("deserialize should succeed");

    println!("  Round-trip verification:");
    println!(
        "    Vector count:  {} (was {})",
        restored.vector_count(),
        include_filter.vector_count()
    );
    println!(
        "    Member count:  {} (was {})",
    ...
    println!(
        "    Vector 250:    {} (was {})",
        restored.contains(250),
        include_filter.contains(250)
    );
    println!(
        "    Vector 500:    {} (was {})",
        restored.contains(500),
        include_filter.contains(500)
    );
    println!();
    ...
    println!(
        "  Total MEMBERSHIP_SEG:     {} bytes (header + bitmap)",
    ...
    println!(
        "  Magic verified:           0x{:08X} == MEMBERSHIP_MAGIC",
        magic
    );
    println!();
