...
impl Registry {
    ...
    fn total(&self) -> u64 {
        self.hand_count + self.tool_count + self.channel_count
    }
...
fn main() {
    ...
    println!("\n--- 2. Register Hands ({}) ---", HANDS.len());
    {
        ...
        for h in HANDS {
            println!(
                "    {:12} {:22} tier={} sec={}",
                h.name, h.domain, h.tier, h.security
            );
        }
    }
    witness(
        &mut wit,
        &format!("REGISTER_HANDS:{}", HANDS.len()),
        1_709_000_000_000_000_000,
        0x01,
    );
    ...
    witness(
        &mut wit,
        &format!("REGISTER_TOOLS:{}", TOOLS.len()),
        1_709_000_001_000_000_000,
        0x01,
    );
    ...
    witness(
        &mut wit,
        &format!("REGISTER_CHANNELS:{}", CHANNELS.len()),
        1_709_000_002_000_000_000,
        0x01,
    );
    ...
    println!("\n--- 6. Quality Envelope ---");
    let envelope = store
        .query_with_envelope(&query, K, &QueryOptions::default())
        .expect("envelope query");
    println!("  Quality: {:?}", envelope.quality);
    println!(
        "  HNSW candidates: {}",
        envelope.evidence.hnsw_candidate_count
    );
    println!(
        "  Safety-net candidates: {}",
        envelope.evidence.safety_net_candidate_count
    );
    println!("  Budget total_us: {}", envelope.budgets.total_us);
    println!(
        "  Results: {} (top match: id={}, d={:.4})",
    ...
    println!("\n--- 7. Audited Query ---");
    let audited = store
        .query_audited(&query, K, &QueryOptions::default())
        .expect("audited query");
    println!(
        "  Returned {} results (witness auto-appended to store)",
        audited.len()
    );
    println!(
        "  Store witness hash: {}",
        hex(&store.last_witness_hash()[..8])
    );
    witness(
        &mut wit,
        "QUERY_AUDITED:k=5",
        1_709_000_012_000_000_000,
        0x02,
    );
    ...
    println!("\n--- 11. Membership Filter (tenant isolation) ---");
    {
        ...
        println!(
            "  Created include-mode filter: {} members of {} total",
            mf.member_count(),
            reg.total()
        );
        println!(
        ...
        println!(
            "  Tool 'http_fetch' visible: {}  (expect true)",
            tool_visible
        );
        assert!(!hand_visible);
        assert!(tool_visible);
    ...
    witness(
        &mut wit,
        "MEMBERSHIP_FILTER:tenant",
        1_709_000_015_000_000_000,
        0x01,
    );
    ...
    println!("\n--- 12. DoS Hardening ---");
    {
        ...
        match bucket.try_consume(cost) {
            Ok(remaining) => println!(
                "  Token bucket: consumed {} ops, {} remaining",
                cost, remaining
            ),
        ...
        println!(
            "  Negative cache: hit1={} hit2={} hit3(blacklist)={}",
            bl1, bl2, bl3
        );
        println!(
            "  Signature blacklisted: {}",
            neg_cache.is_blacklisted(&sig)
        );

        // Proof-of-work
        let pow = ProofOfWork {
            challenge: [
                0x4F, 0x50, 0x45, 0x4E, 0x46, 0x41, 0x4E, 0x47, // "OPENFANG"
    ...
    witness(
        &mut wit,
        "DOS_HARDENING:bucket+cache+pow",
        1_709_000_016_000_000_000,
        0x01,
    );
    ...
    let wasm_seg = store
        .embed_wasm(
            ...
            fake_wasm, 1, // export_count
            1, // bootstrap_priority
            0, // interpreter_type
        )
        .expect("embed wasm");
    println!(
        "  Embedded WASM microkernel: seg_id={}, {} bytes",
        wasm_seg,
        fake_wasm.len()
    );
    ...
    let (hdr, bytecode) = store
        .extract_wasm()
        .expect("extract wasm")
        .expect("wasm present");
    println!(
        "  Extracted: header={} bytes, bytecode={} bytes",
        hdr.len(),
        bytecode.len()
    );
    ...
    witness(
        &mut wit,
        "EMBED_WASM:microkernel",
        1_709_000_018_000_000_000,
        0x01,
    );
    ...
    let kern_seg = store
        ...
        .expect("embed kernel");
    println!(
        "  Embedded kernel: seg_id={}, {} bytes, port=8080",
        kern_seg,
        fake_kernel.len()
    );

    let (khdr, kpayload) = store
        .extract_kernel()
        .expect("extract kernel")
        .expect("kernel present");
    println!(
        "  Extracted: header={} bytes, payload={} bytes",
        khdr.len(),
        kpayload.len()
    );
    ...
    let fake_ebpf = &[
        0xb7, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00u8, 0x95, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00,
    ];
    let ebpf_seg = store
        .embed_ebpf(
            ...
            fake_ebpf, None, // no BTF
        )
        .expect("embed ebpf");
    println!(
        "  Embedded eBPF: seg_id={}, {} bytes (2 insns)",
        ebpf_seg,
        fake_ebpf.len()
    );

    let (ehdr, eprog) = store
        .extract_ebpf()
        .expect("extract ebpf")
        .expect("ebpf present");
    println!(
        "  Extracted: header={} bytes, program={} bytes",
        ehdr.len(),
        eprog.len()
    );
...
<p>7 Hands | 38 Tools | 20 Channels</p></body></html>"#;
    ...
    println!(
        "  Embedded dashboard: seg_id={}, {} bytes",
        dash_seg,
        dashboard_html.len()
    );

    let (dhdr, dbundle) = store
        .extract_dashboard()
        .expect("extract dash")
        .expect("dash present");
    println!(
        "  Extracted: header={} bytes, bundle={} bytes",
        dhdr.len(),
        dbundle.len()
    );
    ...
    witness(
        &mut wit,
        "DELETE+COMPACT:twitter",
        1_709_000_020_000_000_000,
        0x01,
    );
    ...
    let child = store
        .derive(&derived_path, DerivationType::Snapshot, None)
        .expect("derive");
    ...
    println!(
        "  Lineage: parent_id matches = {}",
        child_parent == parent_fid
    );
    ...
    let exp_r = staging
        .ingest_batch(&[exp_vec.as_slice()], &[exp_id], Some(&exp_meta))
        .expect("ingest sentinel");
    println!("  Added 'sentinel' to staging (epoch {})", exp_r.epoch);
    println!(
        "  Staging: {} vectors (parent: {})",
        staging.status().total_vectors,
        st_after.total_vectors
    );

    if let Some(stats) = staging.cow_stats() {
        println!(
            "  COW after write: {} clusters, {} local",
            stats.cluster_count, stats.local_cluster_count
        );
    }
    ...
    println!("\n--- 21. AGI Container ---");
    {
        ...
        println!(
            "  Flags: kernel={} orchestrator={} eval={} offline={} tools={}",
            header.has_kernel(),
            header.has_orchestrator(),
            header.flags & 0x10 != 0, // bit 4: eval suite present
            header.is_offline_capable(),
            header.flags & 0x400 != 0
        ); // bit 10: tool registry present
        ...
        println!(
            "  Orchestrator: {} bytes",
            parsed.orchestrator_config.map_or(0, |c| c.len())
        );
        println!(
            "  Tool registry: {} bytes",
            parsed.tool_registry.map_or(0, |c| c.len())
        );
        println!(
            "  Project instructions: {} bytes",
            parsed.project_instructions.map_or(0, |c| c.len())
        );
    }
    witness(
        &mut wit,
        "AGI_CONTAINER:build+parse",
        1_709_000_050_000_000_000,
        0x01,
    );
    ...
    let pre_close = store
        .query(&query, K, &QueryOptions::default())
        .expect("pre-close query");
    let final_st = store.status();
    println!(
        "  Before: {} vectors, {} segments, {} bytes",
    ...
    println!(
        "  After:  {} vectors, epoch {}",
        reopen_st.total_vectors, reopen_st.current_epoch
    );
    println!(
        "  File ID preserved: {}",
        hex(&reopened.file_id()[..8]) == parent_fid
    );

    // Verify segments survive persistence
    let wasm_ok = reopened.extract_wasm().expect("re-extract wasm").is_some();
    let kern_ok = reopened
        .extract_kernel()
        .expect("re-extract kernel")
        .is_some();
    let ebpf_ok = reopened.extract_ebpf().expect("re-extract ebpf").is_some();
    let dash_ok = reopened
        .extract_dashboard()
        .expect("re-extract dash")
        .is_some();
    println!(
        "  WASM={} Kernel={} eBPF={} Dashboard={}",
        wasm_ok, kern_ok, ebpf_ok, dash_ok
    );

    let recheck = reopened
        .query(&query, K, &QueryOptions::default())
        .expect("recheck");
    assert_eq!(
        pre_close.len(),
    ...
    println!(
        "  Segments:     VEC + WASM + KERN + EBPF + DASH = {} total",
        seg_dir.len()
    );
    println!("  Lifecycle:    delete + compact (twitter removed)");
    println!(
        "  Lineage:      derive depth {}, COW branch with sentinel",
        child_depth
    );
    ...
    println!(
        "  File:         {} bytes, {} segments, persistence verified",
