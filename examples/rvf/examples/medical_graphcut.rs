...
fn extract_features(img: &TissueImage) -> Vec<Vec<f32>> {
    ...
    for y in 0..GRID {
        for x in 0..GRID {
            ...
            for dy in -1i32..=1 {
                for dx in -1i32..=1 {
                    let v = img.intensity[(y as i32 + dy).clamp(0, GRID as i32 - 1) as usize
                        * GRID
    ...
    for y in 0..GRID {
        for x in 0..GRID {
            ...
            for &(dx, dy) in &[(1i32, 0i32), (0, 1)] {
                let (nx, ny) = (x as i32 + dx, y as i32 + dy);
                if nx >= GRID as i32 || ny >= GRID as i32 {
                    continue;
                }
...
fn threshold_segment(img: &TissueImage, h: &HealthyModel, thr: f64) -> Vec<bool> {
    img.intensity
        .iter()
        .map(|&v| ((v - h.mean).abs() / h.std.max(1e-6)) > thr)
        .collect()
}
...
fn per_lesion_detection(img: &TissueImage, pred: &[bool]) -> (usize, usize) {
    ...
    for les in &img.lesions {
        ...
        if total > 0 && hit as f64 / total as f64 > 0.5 {
            detected += 1;
        }
    }
...
fn main() {
    ...
    for (mi, &modality) in modalities.iter().enumerate() {
        ...
        println!(
            "  GC     Dice={:.3} Jacc={:.3} Sens={:.3} Spec={:.3} Lesions={}/{}",
            gc_m.dice, gc_m.jaccard, gc_m.sensitivity, gc_m.specificity, gc_ld.0, gc_ld.1
        );
        println!(
            "  Thresh Dice={:.3} Jacc={:.3} Sens={:.3} Spec={:.3} Lesions={}/{}\n",
    ...
    let ing = store
        .ingest_batch(&vrefs, &all_ids, Some(&all_meta))
        .expect("ingest");
    println!(
        "  Ingested: {} embeddings (rejected: {})",
        ing.accepted, ing.rejected
    );
    ...
    for (i, &gt) in ref_img.ground_truth.iter().enumerate() {
        if gt {
            for (j, &v) in ref_embs[i].iter().enumerate() {
                proto[j] += v;
            }
            pc += 1;
        }
    ...
    let sr = store
        .query(&proto, 10, &QueryOptions::default())
        .expect("q");
    println!("  {:>6}  {:>10}  {:>8}", "ID", "Distance", "Modality");
    println!("  {:->6}  {:->10}  {:->8}", "", "", "");
    for r in &sr {
        let ml = match (r.id as usize) / (GRID * GRID) {
            0 => "T1-MRI",
            1 => "T2-MRI",
            2 => "CT",
            _ => "??",
    ...
    println!("\n--- Lineage ---");
    let child = store
        .derive(&tmp.path().join("seg.rvvis"), DerivationType::Filter, None)
        .expect("derive");
    println!(
        "  Parent: {}  Child: {}  Depth: {}",
        hex_string(store.file_id()),
        hex_string(child.parent_id()),
        child.lineage_depth()
    );
    ...
    for r in &results {
        let imp = if r.th.dice > 0.0 {
            (r.gc.dice - r.th.dice) / r.th.dice * 100.0
        } else {
            0.0
        };
        println!(
            "  {:>8} {:>3} {:>6} {:>6.3} {:>6.3} {:>6.3} {:>6.3} {:>3}/{:<3} {:>+7.1}%",
            r.l,
            r.nl,
            "GC",
            r.gc.dice,
    ...
    println!(
        "\n  RVF: {} embeddings | Witness: {} entries | Modalities: {}",
        ing.accepted,
        verified.len(),
        3
    );
